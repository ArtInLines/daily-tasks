...
uint32_t part1(SV text)
{
	...
	uint32_t *copies = calloc(h, sizeof(uint32_t));
	...
	for (size_t i = 0; i < h; i++) {
		...
		while (j < right.len) {
			if (right.str[j] == ' ') j++;
				...
				points += arr[num]; // @Note: arr[num] is either 0 or 1
				j += num_len;
			}
		}

		// printf("Card %lld: %d, points: %d\n", i + 1, copies[i] + 1, points);
		sum += copies[i] + 1;
		for (uint32_t j = 1; j <= points && i + j < h; j++) {
