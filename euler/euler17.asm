...
; From 10 to 19
teensTable    dd 3,6,6,8,8,7,7,9,9,8
; All ten-based words starting at 20 (e.g. 20-30-40-...-90)
tensTable     dd 6,6,6,5,5,7,6,6
; Length of word "and"
andLen        dd 3
...
.next:
    ...
    mov     ecx, dword [ebp - 8]            ; ecx       = upper/10 - 2
    mov     ebx, 10                         ; ebx       = 10
.loop:
    cmp     ecx, 0
    je      .loop_end
    ...
    jmp     .loop
.loop_end:
    mov     dword [ebp - 8], 9              ; [ebp - 8] = 9 = 19 % 10 (to make sure the assumption for .below_twenty is correct)
...
_sum_hundreds:
    ...
    add     dword [ebp - 4], eax
    ; "and" * (upper - 99 - (upper/10))
    mov     eax, dword [andLen]
    sub     ebx, dword [ebp - 8]
    imul    ebx
    add     dword [ebp - 4], eax
    ; sum_tens(99) * upper/10
    mov     dword [ebp - 16], 99
    call    _sum_tens
    imul    dword [ebp - 8]
    add     dword [ebp - 4], eax
...
_sum:
    ...
    cmp     ecx, 1000
    jl      .hundreds
    add     edx, dword [onesTable]
    add     edx, dword [tenPowerTable + 4]
    mov     dword [ebp], edx
    dec     dword [ebp - 4]
.hundreds:
    call    _sum_hundreds
