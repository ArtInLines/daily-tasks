...
// Store the second matrix as a transposed matrix immediately, to increase data-locality for the matrix multiplication

void matrixMul(int n, int* a, int* t, int* c)
{
	for (int i = 0; i < n; i++)
		for (int j = 0; j < n; j++)
			for (int k = 0; k < n; k++)
...
void randTransMatrix(int* matrix, int rows, int cols, int limit)
{
	for (int i = 0; i < rows; i++)
		for (int j = 0; j < cols; j++)
			matrix[j * rows + i] = xorshift() % limit;
}
